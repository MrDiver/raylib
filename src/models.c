...
void DrawModelEx(Model model, Vector3 position, Vector3 rotationAxis, float rotationAngle, Vector3 scale, Color tint)
{
    ...
    Mesh cyl = GenMeshCylinder(0.02,1,4);
    Material defmat = LoadMaterialDefault();
    defmat.maps[MATERIAL_MAP_DIFFUSE].color = RED;
...
static Model LoadOBJ(const char *fileName)
{
    ...
    if (fileText != NULL)
    {
        ...
        model.meshCount = materialCount; //#1809 (#1917)
...
static Model LoadGLTF(const char *fileName)
{
    ...
    *************************************************************************************/
    ...
    if (result == cgltf_result_success)
    {
        ...
        TRACELOG(LOG_DEBUG, "MODEL: [%s] Scene->nodes_count: %i",fileName,data->scene->nodes_count); //TODO: Remove
        for (unsigned int i = 0; i < data->scene->nodes_count; i++)
        {
            GetGLTFPrimitiveCount(data->scene->nodes[i], &primitivesCount);
        }
        TRACELOG(LOG_DEBUG, "MODEL: [%s] primitivesCount: %i",fileName,primitivesCount); //TODO: Remove
        ...
        TRACELOG(LOG_ERROR,"DISABLED BONES");
...
static void InitGLTFBones(Model* model, const cgltf_data* data)
{
    ...
    TRACELOG(LOG_DEBUG, "MODEL: Nodes Count : %i", data->nodes_count);
        ...
        for (unsigned int j = 0; j < mesh->primitives[p].attributes_count; j++)
        {
            if (mesh->primitives[p].attributes[j].type == cgltf_attribute_type_position)
            {
                ...
                for (int v = 0; v < outModel->meshes[(*primitiveIndex)].vertexCount; v++)
                {
                    ...
                    printf("Using Transform on vertex %i : (%f,%f,%f)",v,vertex.x,vertex.y,vertex.z);
...
{
    printf("============= Model: NODE TRANSFORM pi: %i ==============\n", *primitiveIndex);
//     mathPrintM(nodeTransform);
    printf("Node Name: %s\n",node->name);
     if(node->has_matrix){
